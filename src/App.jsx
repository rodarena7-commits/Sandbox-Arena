...
const appId = 'sandeco-ball-v1'; // Cambiado de echo-path-pro-v1
...
const App = () => {
  ...
  // --- FIREBASE & CACHE ---
  useEffect(() => {
    const cached = localStorage.getItem('sandeco_ball_profile'); // Cambiado de echo_path_profile
  ...
  useEffect(() => {
    ...
    getDoc(profileRef).then(snap => {
      if (snap.exists()) {
        ...
        setProfile(data);
        localStorage.setItem('sandeco_ball_profile', JSON.stringify(data)); // Cambiado
        setIsProfileSet(true);
        setGameState('START');
      } else if (!localStorage.getItem('sandeco_ball_profile')) { // Cambiado
        setGameState('SETUP');
      } else {
        setIsProfileSet(true);
        setGameState('START');
  ...
  const levelData = useMemo(() => {
    const generateLevel = (index, mode) => {
      ...
      // UN SOLO PORTAL (entrada y salida) - verde fluor
      const portals = [];
      // Solo crear UN portal por nivel
      ...
      portals.push({ 
        ...
        b: portalB, 
        id: `portal-${index}`,
        used: false // Marcador para saber si ya fue usado
      });
  ...
  const handleExitToMenu = async () => { 
    ...
    setUsedPortals(new Set()); // Limpiar portales usados al salir
  };
  ...
  const saveProfile = async () => { 
    ...
    localStorage.setItem('sandeco_ball_profile', JSON.stringify(profile)); // Cambiado
    setIsProfileSet(true); 
    setGameState('START'); 
  };
  ...
  useEffect(() => {
    if (gameState === 'PLAYING') {
      ...
      setUsedPortals(new Set()); // Limpiar portales usados al empezar nivel
  ...
  useEffect(() => {
    ...
    const winLevel = () => {
      ...
      setHasKey(false);
      setUsedPortals(new Set()); // Limpiar portales usados al ganar nivel
  ...
  }, [gameState, currentLevelData, hasKey, isMuted, profile.avatar, isInShield, gameMode, editorMode, usedPortals]);
  ...
  const restartFromLevel1 = () => { 
    ...
    setHasKey(false); 
    setUsedPortals(new Set()); // Limpiar portales usados al reiniciar
    ...
    return (
      <div className="h-screen bg-slate-950 flex items-center justify-center text-cyan-400 font-black tracking-widest uppercase">
        <div className="text-center">
          <div className="text-4xl mb-4 animate-spin">⚪</div> {/* Cambiado de 🌀 a ⚪ */}
          <div className="animate-pulse">Cargando Sandeco Ball...</div> {/* Cambiado */}
        </div>
      </div>
    );
  }
      ...
      <header className="flex-none p-4 flex flex-col gap-3 bg-slate-900/80 border-b border-slate-800/50 z-50 backdrop-blur-sm">
        <div className="flex justify-between items-center w-full">
          ...
          <h1 className="text-xl font-black italic tracking-tighter uppercase leading-none bg-gradient-to-r from-white to-cyan-400 bg-clip-text text-transparent">
            SANDECO <span className="text-cyan-400">BALL</span> ⚪ {/* Cambiado completamente */}
          </h1>
      ...
      <footer className="flex-none p-4 bg-slate-900/80 border-t border-slate-800/50 flex flex-col items-center gap-3 z-50 text-white backdrop-blur-sm">
        ...
        {gameState === 'PLAYING' && gameMode === 'CREATIVE' && (
          <div className="w-full max-w-[600px] text-center text-xs text-slate-400">
            ...
            <p className="mt-1">🌀 <strong>Portal verde fluor:</strong> Único por nivel, desaparece tras usarse.</p> {/* Texto actualizado */}
          </div>
        )}
        
        {gameState !== 'PLAYING' && (
          <p className="text-[9px] text-slate-500 font-black uppercase tracking-[0.5em] animate-pulse text-center">
            Sandeco Ball ⚪ • Campo de Protección • Portal Único • Editor de Niveles {/* Cambiado */}
          </p>
        )}
        
        {gameState === 'PLAYING' && gameMode !== 'CREATIVE' && (
          <p className="text-[9px] text-slate-500 font-black uppercase tracking-[0.5em] animate-pulse text-center">
            Arrastra el Piloto • Toca para Eco • Campo violeta = Protección • Portal verde fluor (solo un uso) {/* Actualizado */}
          </p>
        )}
      </footer>
    </div>
