...
const SHIELD_RADIUS = 60; // Radio del campo de protección estático
...
const App = () => {
  ...
  const levelData = useMemo(() => {
    const generateLevel = (index, mode) => {
      ...
      // Portales múltiples (3 pares en niveles altos)
      const portals = [];
  ...
  useEffect(() => {
    if (gameState === 'PLAYING') {
      ...
      setIsInShield(Math.hypot(
        playerPos.current.x - (currentLevelData.shield?.x || SHIELD_POSITION.x),
        playerPos.current.y - (currentLevelData.shield?.y || SHIELD_POSITION.y)
      ) < SHIELD_RADIUS);
    }
  ...
  useEffect(() => {
    ...
    const handleCollision = () => {
      ...
      setLives(prev => {
        ...
        if (next <= 0) { 
          ...
          isDragging.current = false;
          setIsInShield(Math.hypot(
            playerPos.current.x - (level.shield?.x || SHIELD_POSITION.x),
            playerPos.current.y - (level.shield?.y || SHIELD_POSITION.y)
          ) < SHIELD_RADIUS);
          return next; 
        }
      });
    ...
    const render = (time) => {
      ...
      level.portals?.forEach(portal => {
        const drawPortal = (p, id) => {
          ...
          if (op > 0) { 
            ...
            for (let i = 0; i < 8; i++) {
              ...
              ctx.fillStyle = `rgba(168, 85, 247, ${op * 0.5})`;
              ctx.fill();
            }
          ...
          if (portalCooldown.current <= 0 && Math.hypot(playerPos.current.x - p.x, playerPos.current.y - p.y) < 25) { 
            ...
            const target = portal.a === p ? portal.b : portal.a;
            if (target) {
        ...
        drawPortal(portal.a, `a-${portal.id}`);
        drawPortal(portal.b, `b-${portal.id}`);
      });
      ...
      // Campo de protección estático (círculo violeta neon)
      const shieldPos = level.shield || SHIELD_POSITION;
      const shieldOp = visibilityMap.current.get('shield') || 0;
      
      if (shieldOp > 0 || gameMode === 'CREATIVE') {
        // Círculo exterior (neón violeta)
        ctx.beginPath();
        ctx.arc(shieldPos.x, shieldPos.y, SHIELD_RADIUS, 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(168, 85, 247, ${shieldOp > 0 ? shieldOp : 0.7})`;
        ctx.lineWidth = 4;
        ctx.stroke();
        
        // Círculo interior brillante
        ctx.beginPath();
        ctx.arc(shieldPos.x, shieldPos.y, SHIELD_RADIUS - 8, 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(192, 132, 252, ${shieldOp > 0 ? shieldOp * 0.9 : 0.5})`;
        ctx.lineWidth = 2;
        ctx.stroke();
        ...
        for (let i = 0; i < particleCount; i++) {
          ...
          const px = shieldPos.x + Math.cos(angle) * (SHIELD_RADIUS - 5);
          const py = shieldPos.y + Math.sin(angle) * (SHIELD_RADIUS - 5);
        ...
        // Texto "PROTECCIÓN" si es visible
        if (shieldOp > 0) {
          ctx.font = 'bold 16px Arial';
          ctx.textAlign = 'center';
          ctx.textBaseline = 'middle';
          ctx.fillStyle = `rgba(168, 85, 247, ${shieldOp})`;
          ctx.fillText('🛡️ PROTECCIÓN', shieldPos.x, shieldPos.y);
        }
        
        if (shieldOp > 0) visibilityMap.current.set('shield', shieldOp - 0.005);
      }

      // Indicador visual si el jugador está dentro del escudo
      if (isInShield && gameMode !== 'CREATIVE') {
        ctx.font = '12px Arial';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'top';
        ctx.fillStyle = '#a855f7';
        ctx.fillText('PROTEGIDO', playerPos.current.x, playerPos.current.y + PLAYER_RADIUS + 10);
      }
      ...
      <footer className="flex-none p-4 bg-slate-900/80 border-t border-slate-800/50 flex flex-col items-center gap-3 z-50 text-white backdrop-blur-sm">
        ...
        {gameState !== 'PLAYING' && (
          <p className="text-[9px] text-slate-500 font-black uppercase tracking-[0.5em] animate-pulse text-center">
            Echo Path Pro ⚪ • Campo de Protección • Portales • Editor de Niveles
          </p>
        )}
        
        {gameState === 'PLAYING' && gameMode !== 'CREATIVE' && (
          <p className="text-[9px] text-slate-500 font-black uppercase tracking-[0.5em] animate-pulse text-center">
            Arrastra el Piloto • Toca para Eco • Campo violeta = Protección • Portales teletransportan
          </p>
        )}
      </footer>
    </div>
