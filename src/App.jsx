...
const PLAYER_RADIUS = 18; // Avatar 50% más grande
...
const App = () => {
  ...
  // Estados de Juego
  const [gameState, setGameState] = useState('LOADING'); // LOADING, SETUP, START, PLAYING, RESPAWN, RANKING, STORE
  ...
  const handleExitToMenu = async () => {
    ...
    setGameState('START'); // Vuelve al menú de 3 botones
  };
  ...
  // --- GENERADOR DE 100 NIVELES (POSICIÓN SEGURA) ---
  const levelData = useMemo(() => {
    ...
    const getSafePos = (walls, radius = 25) => {
      let attempts = 0;
      while (attempts < 150) {
        const x = 80 + Math.random() * (CANVAS_SIZE - 160);
        const y = 80 + Math.random() * (CANVAS_SIZE - 160);
    ...
    return Array.from({ length: 100 }, (_, i) => {
      ...
      for (let j = 0; j < 6 + Math.floor(diff * 18); j++) {
        walls.push({ 
      ...
      const movingWalls = [];
      if (i >= 1) {
        for (let j = 0; j < 1 + Math.floor(diff * 15); j++) {
          movingWalls.push({ 
            x: 150 + Math.random() * 300, 
            y: 150 + Math.random() * 300, 
            w: 25, h: 25, 
            vx: (Math.random() - 0.5) * (6 + diff * 12), 
            vy: (Math.random() - 0.5) * (6 + diff * 12), 
            id: `m-${i}-${j}` 
          });
        }
      }
  ...
  useEffect(() => {
    ...
    const spawnArrow = () => {
      ...
      const speed = 4 + (currentLevel * 0.12);
      let x, y, vx, vy;
    ...
    const render = (time) => {
      ...
      if (time - lastProjectileSpawn.current > Math.max(400, 2600 - currentLevel * 50)) {
        spawnArrow();
      ...
      pulses.current.forEach(p => {
        ...
        const reveal = (o, isC = false) => {
          ...
          if (Math.abs(d - p.r) < 30) visibilityMap.current.set(o.id || 'key', 1);
        };
      ...
      // Cuadrados Rojos (Enemigos Zigzag)
      ...
      level.coins.forEach(c => {
        ...
        if (Math.hypot(playerPos.current.x - c.x, playerPos.current.y - c.y) < 20 + PLAYER_RADIUS/2) { c.collected = true; setCoins(v => v + 1); setTotalScore(v => v + 50); playSound(1000, 'sine', 0.1, 0.1); }
      });
      level.healthPacks.forEach(h => {
        ...
        if (Math.hypot(playerPos.current.x - h.x, playerPos.current.y - h.y) < 22 + PLAYER_RADIUS/2) { h.collected = true; setLives(v => Math.min(v + 1, 15)); playSound(800, 'sine', 0.3, 0.2); }
      });
      level.echoPacks.forEach(e => {
        ...
        if (Math.hypot(playerPos.current.x - e.x, playerPos.current.y - e.y) < 22 + PLAYER_RADIUS/2) { e.collected = true; setEchoes(v => v + 15); playSound(1200, 'sine', 0.3, 0.2); }
      });

      // Llave y Meta
      if (!hasKey) {
        const op = visibilityMap.current.get('key') || 0;
        if (op > 0) { ctx.fillStyle = `rgba(34, 211, 238, ${op})`; ctx.font = '24px Arial'; ctx.textAlign = 'center'; ctx.fillText('🔑', level.key.x, level.key.y); visibilityMap.current.set('key', op - 0.005); }
        if (Math.hypot(playerPos.current.x - level.key.x, playerPos.current.y - level.key.y) < 25 + PLAYER_RADIUS/2) { setHasKey(true); playSound(900, 'triangle', 0.4, 0.2); }
      }
      ctx.strokeStyle = hasKey ? '#fbbf24' : '#1e293b'; ctx.lineWidth = 4; ctx.beginPath(); ctx.arc(level.end.x, level.end.y, 35, 0, Math.PI * 2); ctx.stroke();
      if (hasKey && Math.hypot(playerPos.current.x - level.end.x, playerPos.current.y - level.end.y) < 35 + PLAYER_RADIUS/2) winLevel();
  ...
  if (gameState === 'LOADING') return <div className="h-screen bg-slate-950 flex items-center justify-center text-cyan-400 font-black tracking-widest uppercase">Sandeco Ball...</div>;
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 text-white">
        <div className="bg-slate-900 border border-slate-800 p-8 rounded-[2rem] w-full max-w-md text-center backdrop-blur-xl">
          <h2 className="text-3xl font-black italic mb-6 uppercase tracking-tighter">Piloto</h2>
          <div className="flex flex-col items-center gap-6">
            ...
            <div className="flex gap-2 pb-2 overflow-x-auto w-full justify-center">
              {DEFAULT_AVATARS.map((av, i) => (
                <button key={i} onClick={() => setProfile({ ...profile, avatar: av })} className="w-10 h-10 rounded-full border-2 border-slate-700 overflow-hidden flex-shrink-0"><img src={av} alt="cartoon" /></button>
              ))}
            </div>
            <input 
            ...
            />
            <button onClick={saveProfile} className="w-full py-4 bg-cyan-500 text-slate-950 font-black rounded-xl hover:scale-105 transition-all shadow-lg uppercase tracking-widest">Entrar</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 bg-slate-950 text-white flex flex-col items-center justify-center p-4 font-sans select-none overflow-hidden touch-none">
        ...
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-3">
             ...
             <div className="hidden sm:block">
               <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Jugador</div>
               <div className="font-bold text-sm leading-none">{profile.name}</div>
             </div>
          </div>
          <h1 className="text-2xl font-black italic tracking-tighter uppercase leading-none">SANDECO <span className="text-cyan-400">BALL</span> ⚪</h1>
          <div className="flex gap-2">
            <button onClick={() => setIsMuted(!isMuted)} className="p-2 bg-slate-900 border border-slate-800 rounded-xl hover:bg-slate-800">
               {isMuted ? '🔇' : '🔊'}
            </button>
            <button onClick={toggleFullscreen} className="p-2 bg-slate-900 border border-slate-800 rounded-xl hover:bg-slate-800">
               {isFullscreen ? '⤓' : '⤢'}
            </button>
          </div>
        </div>
      ...
      <div className="relative rounded-[2rem] overflow-hidden border-4 border-slate-800 shadow-2xl bg-black">
        ...
        {gameState === 'START' && (
          <div className="absolute inset-0 bg-slate-950/95 flex flex-col items-center justify-center p-8 text-center backdrop-blur-sm">
            ...
            <p className="text-slate-500 mb-8 max-w-sm text-sm font-medium uppercase tracking-tight">Esquiva proyectiles y encuentra la salida. <br/> El eco revela el peligro oculto.</p>
            <div className="flex flex-col gap-3 w-full max-w-xs">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button onClick={() => setGameState('RANKING')} className="py-3 bg-slate-900 border border-slate-800 rounded-xl font-bold text-xs uppercase tracking-widest text-white">Ranking</button>
                <button onClick={() => setGameState('STORE')} className="py-3 bg-slate-900 border border-slate-800 rounded-xl font-bold text-xs uppercase tracking-widest text-white">Tienda</button>
              </div>
            </div>
        ...
        {gameState === 'RESPAWN' && (
          <div className="absolute inset-0 bg-slate-950/98 flex flex-col items-center justify-center p-8 text-center backdrop-blur-xl">
            ...
            <div className="mb-6 bg-slate-900/50 p-3 rounded-2xl border border-slate-800 text-white">
               <span className="text-xs font-bold text-slate-400 uppercase">Vidas actuales:</span>
               <span className="ml-2 text-xl font-black text-red-500">{lives}</span>
            </div>
            ...
            <div className="flex flex-col gap-3 w-full max-w-xs">
              ...
              <button onClick={restartFromLevel1} className="py-3 bg-white text-slate-900 font-black rounded-xl text-sm uppercase tracking-widest">Reiniciar Nivel 1</button>
              <button onClick={handleExitToMenu} className="py-3 bg-red-600/10 text-red-500 border border-red-500/30 font-black rounded-xl text-sm uppercase tracking-widest">Menú Principal</button>
            </div>
          </div>
        )}
        ...
        {gameState === 'RANKING' && (
          <div className="absolute inset-0 bg-slate-950/98 flex flex-col p-8 backdrop-blur-xl text-white">
             <div className="flex justify-between items-center mb-6">
                <h2 className="text-3xl font-black italic tracking-tighter uppercase text-white">Ránkings</h2>
                <button onClick={() => setGameState('START')} className="p-2 bg-slate-800 rounded-xl text-white font-bold">✕</button>
             </div>
                ...
                {leaderboard.map((p, i) => (
                  <div key={i} className={`flex items-center justify-between p-3 rounded-xl border ${p.uid === user?.uid ? 'bg-cyan-500/10 border-cyan-500/30' : 'bg-slate-900/50 border-slate-800'}`}>
                    <div className="flex items-center gap-3">
                      ...
                      <div><div className="font-bold text-sm text-white">{p.name}</div><div className="text-[10px] text-slate-500 uppercase font-black">Fase {p.level}</div></div>
                    </div>
        ...
        {gameState === 'STORE' && (
          <div className="absolute inset-0 bg-slate-950/98 flex flex-col p-8 backdrop-blur-xl text-white">
             <div className="flex justify-between items-center mb-8">
                <h2 className="text-3xl font-black italic tracking-tighter uppercase text-white">Suministros</h2>
                <button onClick={() => setGameState('START')} className="p-2 bg-slate-800 rounded-xl text-white font-bold">✕</button>
             </div>
             <div className="space-y-4">
               <StoreItem title="Pack de 15 Ecos" price="10 🪙" onBuy={() => { if(coins >= 10) { setCoins(v=>v-10); setEchoes(v=>v+15); } }} />
               <StoreItem title="Vida Extra" price="25 🪙" onBuy={() => { if(coins >= 25) { setCoins(v=>v-25); setLives(v=>Math.min(v+1, 15)); } }} />
               <StoreItem title="Batería Máxima (50 Ecos)" price="30 🪙" onBuy={() => { if(coins >= 30) { setCoins(v=>v-30); setEchoes(v=>v+50); } }} />
             </div>
          </div>
        )}
      </div>
      ...
      <div className="mt-4 w-full max-w-[600px] flex gap-2">
        {gameState === 'PLAYING' && (
          <button 
            ...
            className="w-full py-4 bg-slate-900 border border-slate-800 rounded-2xl font-black text-xs uppercase tracking-[0.2em] text-slate-500 hover:text-white transition-all shadow-xl"
          >
            ← Guardar y Salir
          </button>
        )}
      ...
      {gameState === 'PLAYING' && (
        <p className="mt-3 text-[10px] text-slate-600 font-black uppercase tracking-[0.5em] animate-pulse text-white">Arrastra para Mover • Toca para Eco</p>
      )}
    </div>
  );
};
...
const StatBox = ({ label, value, color = "text-white" }) => (
  <div className="bg-slate-900/95 p-2 rounded-xl border border-slate-800 text-center shadow-inner">
    <div className="text-[7px] sm:text-[9px] text-slate-600 font-black uppercase tracking-widest mb-1">{label}</div>
    <div className={`text-xs sm:text-base font-black ${color} tracking-tighter`}>{value}</div>
  </div>
);
...
const StoreItem = ({ title, price, onBuy }) => (
  <button onClick={onBuy} className="w-full flex justify-between items-center p-5 bg-slate-900 border border-slate-800 rounded-3xl hover:bg-slate-800 transition-all shadow-lg group">
    <div className="text-left">
        ...
        <div className="text-[10px] text-slate-500 uppercase font-black tracking-widest">Equipamiento</div>
    </div>
