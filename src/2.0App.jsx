...
const App = () => {
  ...
  useEffect(() => {
    ...
    const winLevel = () => {
      ...
      setCurrentLevel(next); setMaxLevelReached(v => Math.max(v, next)); setTotalScore(v => v + 1000); setHasKey(false);
      visibilityMap.current.clear(); projectiles.current = []; playerPos.current = { ...levelData[next].start };
  ...
  const buyLife = () => {
    if (coins >= currentRespawnPrice) { setCoins(v => v - currentRespawnPrice); setLives(v => v + 1); setLivesBoughtInSession(v => v + 1); playSound(700, 'sine', 0.2, 0.2); }
  };

  const continueAfterRespawn = () => {
    if (lives > 0) { if (respawnTimerRef.current) clearInterval(respawnTimerRef.current); setGameState('PLAYING'); setLivesBoughtInSession(0); playerPos.current = { ...levelData[currentLevel].start }; visibilityMap.current.clear(); projectiles.current = []; }
  };
      ...
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 overflow-hidden text-white">
        <div className="bg-slate-900 border border-slate-800 p-8 rounded-[2rem] w-full max-w-md text-center backdrop-blur-xl">
          ...
          <div className="flex flex-col items-center gap-6">
            ...
            <button onClick={saveProfile} className="w-full py-4 bg-cyan-500 text-slate-950 font-black rounded-xl hover:scale-105 shadow-lg uppercase tracking-widest">Entrar</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 bg-slate-950 text-white flex flex-col items-center justify-center p-4 font-sans select-none overflow-hidden touch-none">
      <div className="w-full max-w-[600px] mb-4">
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-3">
             <img src={profile.avatar} className="w-12 h-12 rounded-full border-2 border-cyan-500 shadow-lg" alt="p" />
          ...
          <div className="flex gap-2">
            <button onClick={() => setIsMuted(!isMuted)} className="p-2 bg-slate-900 border border-slate-800 rounded-xl hover:bg-slate-800">{isMuted ? '🔇' : '🔊'}</button>
            <button onClick={toggleFullscreen} className="p-2 bg-slate-900 border border-slate-800 rounded-xl hover:bg-slate-800">{isFullscreen ? '⤓' : '⤢'}</button>
          </div>
        </div>
      ...
      <div className="relative rounded-[2rem] overflow-hidden border-4 border-slate-800 shadow-2xl bg-black">
        <canvas ref={canvasRef} width={CANVAS_SIZE} height={CANVAS_SIZE} className="w-full aspect-square max-w-[600px] touch-none cursor-crosshair" />
        ...
        {gameState === 'START' && (
          <div className="absolute inset-0 bg-slate-950/95 flex flex-col items-center justify-center p-8 text-center backdrop-blur-sm">
            <h1 className="text-5xl font-black italic mb-2 uppercase">SANDECO BALL</h1>
            <p className="text-slate-500 mb-8 max-w-sm text-sm uppercase tracking-tight">Esquiva proyectiles y enemigos zigzag. <br/> El eco revela tu destino.</p>
            <div className="flex flex-col gap-3 w-full max-w-xs">
              <button onClick={() => setGameState('PLAYING')} className="py-5 bg-cyan-500 text-slate-950 font-black rounded-3xl text-xl hover:scale-105 shadow-xl shadow-cyan-500/30 uppercase tracking-widest">Jugar</button>
        ...
        {gameState === 'RESPAWN' && (
          <div className="absolute inset-0 bg-slate-950/98 flex flex-col items-center justify-center p-8 text-center backdrop-blur-xl">
            ...
            <div className="flex flex-col gap-3 w-full max-w-xs">
              <button onClick={buyLife} disabled={coins < currentRespawnPrice} className={`py-4 rounded-xl font-black text-lg transition-all shadow-lg ${coins >= currentRespawnPrice ? 'bg-amber-500 text-slate-950 hover:scale-105' : 'bg-slate-800 text-slate-600'}`}>{coins >= currentRespawnPrice ? `+1 VIDA (${currentRespawnPrice} 🪙)` : `FALTAN ${currentRespawnPrice - coins} 🪙`}</button>
              <button onClick={continueAfterRespawn} disabled={lives === 0} className={`py-4 font-black rounded-xl text-lg uppercase tracking-widest shadow-md transition-all ${lives > 0 ? 'bg-cyan-500 text-slate-950 hover:scale-105' : 'bg-slate-800 text-slate-600 opacity-50 cursor-not-allowed'}`}>Continuar Nivel</button>
        ...
        {gameState === 'RANKING' && (
          <div className="absolute inset-0 bg-slate-950/98 flex flex-col p-8 backdrop-blur-xl text-white">
             <div className="flex justify-between items-center mb-6"><h2 className="text-3xl font-black italic tracking-tighter uppercase text-white">Líderes</h2><button onClick={() => setGameState('START')} className="p-2 bg-slate-800 rounded-xl text-white font-bold hover:bg-slate-700">✕</button></div>
             <div className="space-y-2 overflow-y-auto pr-2 flex-1 scrollbar-hide">
                {leaderboard.map((p, i) => (
                  <div key={i} className={`flex items-center justify-between p-3 rounded-xl border ${p.uid === user?.uid ? 'bg-cyan-500/10 border-cyan-500/30 shadow-inner' : 'bg-slate-900/50 border-slate-800'}`}>
                    <div className="flex items-center gap-3">
                      <span className="w-5 font-black text-slate-600 text-xs">{i + 1}</span>
                      <img src={p.avatar} className="w-8 h-8 rounded-full" alt="" />
                      <div className="text-left"><div className="font-bold text-sm text-white leading-none">{p.name}</div><div className="text-[10px] text-slate-500 uppercase font-black">Nivel {p.level}</div></div>
                    </div>
        ...
        {gameState === 'STORE' && (
          <div className="absolute inset-0 bg-slate-950/98 flex flex-col p-8 backdrop-blur-xl text-white">
             <div className="flex justify-between items-center mb-8"><h2 className="text-3xl font-black italic tracking-tighter uppercase text-white">Almacén</h2><button onClick={() => setGameState('START')} className="p-2 bg-slate-800 rounded-xl text-white font-bold hover:bg-slate-700">✕</button></div>
      ...
      <div className="mt-4 w-full max-w-[600px] flex gap-2">
        {gameState === 'PLAYING' && (
          <button onClick={handleExitToMenu} className="w-full py-4 bg-slate-900 border border-slate-800 rounded-2xl font-black text-xs uppercase tracking-[0.2em] text-slate-500 hover:text-white transition-all shadow-xl hover:bg-slate-800">← Guardar y Salir</button>
        )}
      </div>
      {gameState === 'PLAYING' && <p className="mt-3 text-[10px] text-slate-600 font-black uppercase tracking-[0.5em] animate-pulse text-white">Arrastra el Piloto • Pulsa para Eco</p>}
    </div>
  );
};
...
const StoreItem = ({ title, price, onBuy }) => (
  <button onClick={onBuy} className="w-full flex justify-between items-center p-5 bg-slate-900 border border-slate-800 rounded-3xl hover:bg-slate-800 transition-all shadow-lg group">
    <div className="text-left"><div className="font-bold text-sm text-white group-hover:text-cyan-400 transition-colors">{title}</div><div className="text-[10px] text-slate-500 uppercase font-black tracking-widest leading-none">Equipamiento</div></div>
