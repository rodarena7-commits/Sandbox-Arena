...
const SHIELD_RADIUS = PLAYER_RADIUS * 1.5; // Campo del mismo tamaño que el avatar
const SHIELD_POSITION = { x: 300, y: 300 }; // Posición fija del escudo
...
const App = () => {
  ...
  // Modo Creativo
  const [editorMode, setEditorMode] = useState('SELECT'); // SELECT, WALL, ENEMY, PORTAL, ITEM
  ...
  // --- FIREBASE & CACHE ---
  useEffect(() => {
    const cached = localStorage.getItem('echo_path_profile');
  ...
  useEffect(() => {
    ...
    getDoc(profileRef).then(snap => {
      if (snap.exists()) {
        ...
        setProfile(data);
        localStorage.setItem('echo_path_profile', JSON.stringify(data));
        setIsProfileSet(true);
        setGameState('START');
      } else if (!localStorage.getItem('echo_path_profile')) {
        setGameState('SETUP');
      } else {
        setIsProfileSet(true);
        setGameState('START');
  ...
  const levelData = useMemo(() => {
    const generateLevel = (index, mode) => {
        ...
        if (index >= 10) {
          ...
          for (let j = 0; j < crossCount; j++) {
            campaignFeatures.crosses.push({ 
              ...getSafePos(100, 150), // Mayor radio por ser más grande
              size: 90 + rnd() * 60, // 3x más grande (30-50 original)
              angle: rnd() * Math.PI * 2,
              speed: (easyMode ? 0.003 : mediumMode ? 0.004 : 0.005), // 3x más lento
              id: `cr-${index}-${j}` 
            });
          }
        }
      ...
      // Portales múltiples (SIEMPRE VISIBLES, círculos verde fluor en extremos)
      const portals = [];
      const portalCount = easyMode ? 2 : mediumMode ? 3 : 4;
      for (let j = 0; j < portalCount; j++) {
        // Posicionar portales en extremos opuestos
        let portalA, portalB;
        if (j === 0) {
          // Primer par: superior izquierdo - inferior derecho
          portalA = { x: 80 + rnd() * 100, y: 80 + rnd() * 100 };
          portalB = { x: 420 + rnd() * 100, y: 420 + rnd() * 100 };
        } else if (j === 1) {
          // Segundo par: superior derecho - inferior izquierdo
          portalA = { x: 420 + rnd() * 100, y: 80 + rnd() * 100 };
          portalB = { x: 80 + rnd() * 100, y: 420 + rnd() * 100 };
        } else if (j === 2) {
          // Tercer par: izquierda - derecha
          portalA = { x: 80 + rnd() * 100, y: 250 + rnd() * 100 };
          portalB = { x: 420 + rnd() * 100, y: 250 + rnd() * 100 };
        } else {
          // Cuarto par: arriba - abajo
          portalA = { x: 250 + rnd() * 100, y: 80 + rnd() * 100 };
          portalB = { x: 250 + rnd() * 100, y: 420 + rnd() * 100 };
        }
        ...
        portals.push({ a: portalA, b: portalB, id: `portal-${index}-${j}` });
      }
      ...
      // Campo de protección estático (posicionado cerca del inicio)
  ...
  const saveProfile = async () => { 
    ...
    localStorage.setItem('echo_path_profile', JSON.stringify(profile)); 
    setIsProfileSet(true); 
    setGameState('START'); 
  };
  ...
  useEffect(() => {
    if (gameState === 'PLAYING') {
      ...
      setIsInShield(true);
      // Forzar que el campo de protección sea visible desde el inicio
  ...
  const handleEditorClick = (e) => {
    ...
    switch (editorMode) {
      ...
      case 'SHIELD':
        ...
        break;
      case 'DELETE':
        // Eliminar el objeto más cercano
        const allObjects = [
  ...
  useEffect(() => {
    ...
    const handleInputMove = (e) => {
      ...
      // Verificar si está en el campo de protección
    ...
    const handleCollision = () => {
      // Si está dentro del campo de protección, no perder vida
      if (isInShield) {
      ...
      setLives(prev => {
        ...
        if (next <= 0) { 
          ...
          pulses.current = []; 
          isDragging.current = false;
          setIsInShield(true); // Volver al escudo visible al respawnear
          visibilityMap.current.set('shield', 1); // Forzar visibilidad del escudo
          return next; 
        }
      });
    };
    ...
    const render = (time) => {
      ...
      // Solo spawnear proyectiles si no es modo creativo
      ...
      level.walls?.forEach(w => {
        ...
        // Solo colisión si no es modo creativo
        if (gameMode !== 'CREATIVE') {
      ...
      level.crosses?.forEach(c => {
        ...
        if (op > 0) {
          ...
          ctx.lineWidth = 15; // Más grueso por ser más grande
          ctx.beginPath(); 
          ctx.moveTo(-c.size, 0); 
          ctx.lineTo(c.size, 0); 
          ctx.moveTo(0, -c.size); 
      ...
      // PORTALES MÚLTIPLES - SIEMPRE VISIBLES (CÍRCULOS VERDE FLUOR)
      level.portals?.forEach(portal => {
        const drawPortal = (p, id, isPortalA) => {
          // SIEMPRE VISIBLE - no depende de visibilityMap
          ...
          ctx.strokeStyle = `rgba(0, 255, 127, ${pulseIntensity})`; // Verde fluor
          ctx.lineWidth = 6; 
          ctx.stroke();
          ...
          // Partículas verdes fluor girando
          ...
          // Texto indicando si es entrada o salida
          ctx.rotate(-rotation);
          ctx.font = 'bold 10px Arial';
          ctx.textAlign = 'center';
          ctx.textBaseline = 'middle';
          ctx.fillStyle = '#ffffff';
          ctx.fillText(isPortalA ? 'ENTRADA' : 'SALIDA', 0, 0);
          ...
          // Teletransportación al tocar el portal
          if (portalCooldown.current <= 0 && Math.hypot(playerPos.current.x - p.x, playerPos.current.y - p.y) < 30) { 
            // Encontrar el portal pareado
            const target = isPortalA ? portal.b : portal.a;
            if (target) {
              playerPos.current = { ...target }; 
              portalCooldown.current = 60; 
              playSound(800, 'sine', 0.4, 0.2); // Sonido diferente para portales verdes
        ...
        drawPortal(portal.a, `a-${portal.id}`, true);
        drawPortal(portal.b, `b-${portal.id}`, false);
      });
      
      if (portalCooldown.current > 0) portalCooldown.current--;

      // Flechas/proyectiles (solo en modos de juego)
      if (gameMode !== 'CREATIVE') {
      ...
      // Campo de protección estático - SIEMPRE VISIBLE
      const shieldPos = level.shield || SHIELD_POSITION;
      
      // Dibujar campo de protección (siempre visible cuando el jugador está dentro)
      if (isInShield) {
        // Círculo exterior pulsante (neón violeta)
      ...
      // Avatar del jugador - DIBUJAR DESPUÉS del campo para que el campo quede encima
      ctx.save(); 
      ctx.beginPath(); 
      ...
      // Borde del avatar (color diferente si está protegido)
      ...
      // Ahora dibujar campo de protección ENCIMA del avatar si está activo
      if (isInShield) {
        // Dibujar un anillo más pequeño encima del avatar para mayor visibilidad
        ctx.beginPath();
        ...
        // Dibujar texto "PROTEGIDO" encima del campo
  ...
  }, [gameState, currentLevelData, hasKey, isMuted, profile.avatar, isInShield, gameMode, editorMode]);
    ...
    return (
      <div className="h-screen bg-slate-950 flex items-center justify-center text-cyan-400 font-black tracking-widest uppercase">
        <div className="text-center">
          ...
          <div className="animate-pulse">Cargando Echo Path Pro...</div>
        </div>
      </div>
    );
  }
      ...
      <header className="flex-none p-4 flex flex-col gap-3 bg-slate-900/80 border-b border-slate-800/50 z-50 backdrop-blur-sm">
        <div className="flex justify-between items-center w-full">
          ...
          <h1 className="text-xl font-black italic tracking-tighter uppercase leading-none bg-gradient-to-r from-white to-cyan-400 bg-clip-text text-transparent">
            ECHO <span className="text-cyan-400">PATH</span> PRO
          </h1>
          ...
          {gameState === 'START' && (
            <div className="absolute inset-0 bg-slate-950/95 flex flex-col items-center justify-center p-8 text-center backdrop-blur-md z-40 rounded-2xl text-white">
              ...
              <p className="text-slate-400 mb-8 max-w-xs text-xs font-bold uppercase tracking-widest leading-relaxed">
                Campo de protección • Portales múltiples • Cruces 3x • Creador de niveles
              </p>
      ...
      <footer className="flex-none p-4 bg-slate-900/80 border-t border-slate-800/50 flex flex-col items-center gap-3 z-50 text-white backdrop-blur-sm">
        ...
        {gameState === 'PLAYING' && gameMode === 'CREATIVE' && (
          <div className="w-full max-w-[600px] text-center text-xs text-slate-400">
            ...
            <p className="mt-1">🌀 <strong>Portales verde fluor:</strong> Siempre visibles en extremos opuestos.</p>
          </div>
        )}
        
        {gameState !== 'PLAYING' && (
          <p className="text-[9px] text-slate-500 font-black uppercase tracking-[0.5em] animate-pulse text-center">
            Echo Path Pro ⚪ • Campo de Protección • Portales Verde Fluor • Editor de Niveles
          </p>
        )}
        
        {gameState === 'PLAYING' && gameMode !== 'CREATIVE' && (
          <p className="text-[9px] text-slate-500 font-black uppercase tracking-[0.5em] animate-pulse text-center">
            Arrastra el Piloto • Toca para Eco • Campo violeta = Protección • Portales verde fluor teletransportan
          </p>
        )}
      </footer>
    </div>
